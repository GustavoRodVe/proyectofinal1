...
export const ManageAccess = ({
  ...
  if (isLoading || isLoadingOwner) return <div>Cargando...</div>;
  ...
  const handleRoleChange = async (memberId: string, newRole: Role) => {
    try {
      ...
      toast({
        title: 'Éxito',
        description: 'Rol del miembro actualizado correctamente',
      });
  ...
  return (
    <div>
      {can(ProjectAction.INVITE_MEMBERS) && (
        <h1 className="text-xl mb-4">Gestionar acceso</h1>
      )}
          ...
          {canRemoveMembers && (
            <div className="flex items-center gap-4">
              ...
              <span className="text-xs">
                {selectedIds.size} de {totalMemberCount} miembros
              </span>
            </div>
          )}
