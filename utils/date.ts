...
export function formatRelativeTime(date: Date | string): string {
  ...
  if (diff < MINUTE) {
    ...
    return seconds <= 1 ? 'just now' : `${seconds} `;
  }

  // Less than an hour
  if (diff < HOUR) {
    const minutes = Math.floor(diff / MINUTE);
    return minutes === 1 ? '1 min ago' : `hace ${minutes} minutos`;
  }

  // Less than a day
  if (diff < DAY) {
    const hours = Math.floor(diff / HOUR);
    return hours === 1 ? '1 hr ago' : `hace${hours} horas`;
  }

  // Less than a week
  if (diff < WEEK) {
    const days = Math.floor(diff / DAY);
    return days === 1 ? 'yesterday' : `hace${days} dias`;
  }
